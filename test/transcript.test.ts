...
describe('slash commands echoed by the CLI', () => {
  const recs = parseRecords([
    user(1, '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>', { isMeta: true }),
    user(1, '<command-name>/model</command-name>\n            <command-message>model</command-message>\n            <command-args>opus[1m]</command-args>'),
    user(1, '<local-command-stdout>Set model to claude-opus-5[1m]</local-command-stdout>'),
    user(2, [{ type: 'text', text: 'now the real prompt' }]),
  ].join('\n'));
  const t = buildTranscript(recs);
  it('is a command turn carrying its stdout; the caveat is hidden housekeeping', () => {
    expect(t.turns.map(x => [x.promptKind, x.prompt])).toEqual([['command', '/model opus[1m]'], ['user', 'now the real prompt']]);
    expect(t.turns[0]!.items).toEqual([{ kind: 'text', ts: T0 + 1000, text: 'Set model to claude-opus-5[1m]' }]);
    expect(t.hidden).toBe(1);
  });
  it('never becomes the title', () => {
    expect(firstPrompt(recs)).toBe('now the real prompt');
  });
});
